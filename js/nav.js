...
(async function () {
   ...
   function showLoggedOut() {
         const existingMembers = document.getElementById("nav-members-link");
...
// The members directory is only useful once you're signed in, so the
// link is added then rather than sitting in every page's markup.
function addMembersLink() {
  if (document.getElementById("nav-members-link")) return;
  const nav = document.querySelector(".main-nav");
  if (!nav) return;
  const li = document.createElement("li");
  li.id = "nav-members-link";
  li.innerHTML = '<a href="members.html">Members</a>';
