  ...
  const row = p => {
    ...
    return `
      <div class="entry-row" data-player-id="${p.id}">
        ...
        <input type="number" step="1" placeholder="Gross" class="gross-input" value="${ex?.gross_score ?? ''}"><input type="number" step="1" placeholder="Thru" class="thru-input" value="${ex?.thru ?? 18}"><input type="number" step="1" placeholder="Grs pts" class="grosspts-input" value="${ex?.gross_points ?? ''}">
...
async function saveResults() {
  ...
  rows.forEach(row => {
    ...
    upserts.push({
      ...
      gross_score: row.querySelector(".gross-input").value || null, thru: row.querySelector(".thru-input").value || null, gross_points: row.querySelector(".grosspts-input").value || null,
